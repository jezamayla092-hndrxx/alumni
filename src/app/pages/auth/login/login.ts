...
export class Login {
  ...
  async onLogin(): Promise<void> {
    ...
    try {
      ...
      const userStatus = this.getAlumniVerificationStatus(user);

      /*
        Important:
        Pending / under review / rejected alumni must be handled BEFORE disabled.
        Some pending accounts may still have isActive: false from old signup data,
        so checking disabled first causes the wrong "Account Disabled" popup.
      */
          ...
          html: remarks
            ...
            : `
              <p style="margin:0;color:#374151;font-size:0.95rem;">
                Your alumni verification request has been rejected. Please correct your information and resubmit for appeal.
              </p>
            `,
      ...
      if (user.isActive === false || this.getAccountStatus(user) === 'disabled') {
        this.stopLoading();
  ...
  private getAlumniVerificationStatus(user: any): string {
    const rawStatus = String(user.verificationStatus || user.status || '')
      .toLowerCase()
      .trim();
  ...
  private getAccountStatus(user: any): string {
    return String(user.accountStatus || '').toLowerCase().trim();
  }
